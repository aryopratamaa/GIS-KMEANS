	...
	<section id="contact" class="contact">
		<div class="container" data-aos="fade-up">
			...
			<ol class="breadcrumb" style="padding: 20px; box-shadow: 2px 2px 10px #888888; background-color: whitesmoke;">
				...
				<li class="breadcrumb-item active" aria-current="page" style="padding-top:5px;">K-MEANS CLUSTERING LAKALANTAS</li>
			</ol>
					...
					//untuk menampilkan data alternatif
						...
						//untuk menampilkan nilai sub berdasarkan kriteria
								...
								while($result = mysqli_fetch_array($query)){
									...
									$query1 = mysqli_query($conn,"SELECT nilai as sub FROM tbl_lakalantas WHERE id_kecamatan='".$kode."' AND id_kriteria='".$result['id_kriteria']."' ORDER BY id_kriteria ");
									$result1 = mysqli_fetch_array($query1);

									$query2 = mysqli_query($conn,"SELECT nilai as nilai_sub FROM tbl_lakalantas WHERE  id_kriteria='".$result['id_kriteria']."' AND id_kecamatan='4'");
									$result2 = mysqli_fetch_array($query2);
									$distance1 = pow(($result1['sub']-$result2['nilai_sub']),2);
									$sum1+= ($distance1);
									$akr1 = sqrt($sum1);

									$query3 = mysqli_query($conn,"SELECT nilai as nilai_sub FROM tbl_lakalantas WHERE  id_kriteria='".$result['id_kriteria']."' AND id_kecamatan='8'");
									$result3 = mysqli_fetch_array($query3);
									$distance2 = pow(($result1['sub']-$result3['nilai_sub']),2);
									$sum2+= ($distance2);
									$akr2 = sqrt($sum2);

									$query4 = mysqli_query($conn,"SELECT nilai as nilai_sub FROM tbl_lakalantas WHERE  id_kriteria='".$result['id_kriteria']."' AND id_kecamatan='12'");
...
$existing_data_query = mysqli_query($conn, "SELECT * FROM tbl_cluster_lakalantas WHERE id_kecamatan = '$kode'");
if (mysqli_num_rows($existing_data_query) > 0) {
	...
	$query = mysqli_query($conn, "INSERT INTO tbl_cluster_lakalantas (id_kecamatan, c1, c2, c3, group_cluster, iterasi) VALUES ('$kode','$akr1', '$akr2', '$akr3', '$nama_terkecil', '1')");
}
	...
	<div class="table-responsive">
        ...
        // Loop untuk menampilkan nilai sub berdasarkan kriteria dan hasil SUM per cluster
				$query1 = mysqli_query($conn, "SELECT n.id_kriteria, AVG(n.nilai) AS total_nilai 
					FROM tbl_lakalantas n, tbl_cluster_lakalantas k WHERE n.id_kecamatan = k.id_kecamatan AND
			...
			while ($a=mysqli_fetch_array($data)) {
				...
				while($result = mysqli_fetch_array($query)){
					...
					$query2 = mysqli_query($conn, "SELECT n.id_kriteria, AVG(n.nilai) AS nilai_sub 
						FROM tbl_lakalantas n, tbl_cluster_lakalantas k WHERE n.id_kecamatan = k.id_kecamatan AND n.id_kriteria='".$result['id_kriteria']."' AND k.group_cluster = 'C1' AND iterasi='$i' GROUP BY n.id_kriteria ORDER BY n.id_kriteria");
					$result2 = mysqli_fetch_array($query2);
					$distance1 = pow(($result1['sub']-$result2['nilai_sub']),2);
					$sum1+= ($distance1);
					$akr1 = sqrt($sum1);
					...
					$query3 = mysqli_query($conn, "SELECT n.id_kriteria, AVG(n.nilai) AS nilai_sub 
						FROM tbl_lakalantas n, tbl_cluster_lakalantas k WHERE n.id_kecamatan = k.id_kecamatan AND n.id_kriteria='".$result['id_kriteria']."' AND k.group_cluster = 'C2' AND iterasi='$i' GROUP BY n.id_kriteria ORDER BY n.id_kriteria");
					$result3 = mysqli_fetch_array($query3);
					$distance2 = pow(($result1['sub']-$result3['nilai_sub']),2);
					$sum2+= ($distance2);
					$akr2 = sqrt($sum2);
					...
					$query4 = mysqli_query($conn, "SELECT n.id_kriteria, AVG(n.nilai) AS nilai_sub 
						FROM tbl_lakalantas n, tbl_cluster_lakalantas k WHERE n.id_kecamatan = k.id_kecamatan AND n.id_kriteria='".$result['id_kriteria']."' AND k.group_cluster = 'C3' AND iterasi='$i' GROUP BY n.id_kriteria ORDER BY n.id_kriteria");
					$result4 = mysqli_fetch_array($query4);
					$distance3 = pow(($result1['sub']-$result4['nilai_sub']),2);
					$sum3+= ($distance3);
					$akr3 = sqrt($sum3);
...
mysqli_query($conn, "UPDATE tbl_kecamatan SET cluster_lakalantas = '$nama_terkecil' WHERE id_kecamatan = '$kode'");

	// Cek apakah data dengan id_kecamatan sudah ada dalam tabel tbl_cluster
$existing_data_query = mysqli_query($conn, "SELECT * FROM tbl_cluster_lakalantas WHERE id_kecamatan = '$kode' AND iterasi='$ii'");
if (mysqli_num_rows($existing_data_query) > 0) {
    // Data sudah ada, Anda bisa memutuskan apakah akan mengganti data yang sudah ada atau mengabaikannya.
    // Misalnya, Anda ingin mengganti data yang sudah ada:
	$row = mysqli_fetch_assoc($existing_data_query);
    ...
    // Data belum ada, lakukan operasi INSERT untuk memasukkan data baru ke dalam tabel
	$query = mysqli_query($conn, "INSERT INTO tbl_cluster_lakalantas (id_kecamatan, c1, c2, c3, group_cluster, iterasi) VALUES ('$kode','$akr1', '$akr2', '$akr3', '$nama_terkecil', '$ii')");
}
...
mysqli_query($conn, "UPDATE tbl_kecamatan SET ket_lakalantas= '$keterangan' WHERE id_kecamatan = '$kode'");
