					...
					<?php } else { ?>
						...
						<div class="table-responsive">
							<table class="table table-bordered">
								...
								$kecamatanToShow = ['KN03', 'KN06', 'KN09'];

								// Loop untuk menampilkan data alternatif
								$data = mysqli_query($conn, "SELECT * FROM tbl_kecamatan WHERE id_kecamatan IN ('KN03', 'KN06', 'KN09') ORDER BY id_kecamatan");
								$no = 1;
								while ($a = mysqli_fetch_array($data)) {
									$nomor = $no++;
									$kode = $a['id_kecamatan'];
								...
								while ($a = mysqli_fetch_array($data)) {
									...
									<td>$nama</td>";
										...
										while ($result = mysqli_fetch_array($query)) {
											...
											$query2 = mysqli_query($conn, "SELECT nilai as nilai_sub FROM tbl_kriminalitas WHERE  id_kriteria='" . $result['id_kriteria'] . "' AND id_kecamatan='KN03'");
											$result2 = mysqli_fetch_array($query2);
											$distance1 = pow(($result1['sub'] - $result2['nilai_sub']), 2);
											$sum1 += ($distance1);
											$akr1 = sqrt($sum1);

											$query3 = mysqli_query($conn, "SELECT nilai as nilai_sub FROM tbl_kriminalitas WHERE  id_kriteria='" . $result['id_kriteria'] . "' AND id_kecamatan='KN06'");
											$result3 = mysqli_fetch_array($query3);
											$distance2 = pow(($result1['sub'] - $result3['nilai_sub']), 2);
											$sum2 += ($distance2);
											$akr2 = sqrt($sum2);

											$query4 = mysqli_query($conn, "SELECT nilai as nilai_sub FROM tbl_kriminalitas WHERE  id_kriteria='" . $result['id_kriteria'] . "' AND id_kecamatan='KN09'");
				...
				<td class='text-center'>Cluster $cluster</td>";
					...
					<td>$nama</td>";
							...
							FROM tbl_kriminalitas n, tbl_cluster_kriminalitas k WHERE n.id_kecamatan = k.id_kecamatan AND n.id_kriteria='" . $result['id_kriteria'] . "' AND k.group_cluster = 'C3' AND iterasi='$i' GROUP BY n.id_kriteria ORDER BY n.id_kriteria");
											...
											mysqli_query($conn, "UPDATE tbl_kecamatan SET c1='$akr1', c2='$akr2', c3='$akr3', cluster_kriminalitas = '$nama_terkecil' WHERE id_kecamatan = '$kode'");
